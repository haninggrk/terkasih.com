...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .footer-brand:hover { background: #1a1614; color: #faf9f7; border-color: #1a1614; }
...
<div class="page">
    ...
    <section class="hero" id="top">
        ...
        <div class="hero-florals" aria-hidden="true">
            ...
            <div class="portrait-ring">
                @php
                    $photoUrl = null;
                    $photoPath = $memorialPage->photo_path ?? null;
                    if ($photoPath && file_exists(public_path('storage/' . $photoPath))) {
                        $photoUrl = asset('storage/' . $photoPath);
                    } else {
                        $knownPhotos = [
                            'ericpramono' => 'images/eric.jpg',
                            'dini-carolina' => 'images/dini.png',
                        ];
                        $legacyPath = $knownPhotos[$memorialPage->slug] ?? null;
                        if ($legacyPath && file_exists(public_path($legacyPath))) {
                            $photoUrl = asset($legacyPath);
                        } elseif (file_exists(public_path('images/' . $memorialPage->slug . '.jpg'))) {
                            $photoUrl = asset('images/' . $memorialPage->slug . '.jpg');
                        } elseif (file_exists(public_path('images/' . $memorialPage->slug . '.png'))) {
                            $photoUrl = asset('images/' . $memorialPage->slug . '.png');
                        }
                    }
                @endphp
                @if ($photoUrl)
        ...
        <div class="hero-body">
            <h1 class="person-name">{{ $memorialPage->person_name }}</h1>

            @php
                $birth = $memorialPage->birth_date ? \Carbon\Carbon::parse($memorialPage->birth_date)->locale('id')->isoFormat('D MMMM YYYY') : null;
                $death = $memorialPage->death_date ? \Carbon\Carbon::parse($memorialPage->death_date)->locale('id')->isoFormat('D MMMM YYYY') : null;
            @endphp
            @if ($birth || $death)
                <p class="date-range">{{ $birth ?? '…' }} &nbsp;–&nbsp; {{ $death ?? '…' }}</p>
            @endif

            @if ($memorialPage->verse_text_id || $memorialPage->verse_reference)
                <div class="verse-block">
                    @if ($memorialPage->verse_text_id)
                        <p class="verse-text">&ldquo;{{ $memorialPage->verse_text_id }}&rdquo;</p>
                    @endif
                    @if ($memorialPage->verse_reference)
                        <p class="verse-ref">({{ $memorialPage->verse_reference }})</p>
                    @endif
                </div>
            @endif

            @if ($memorialPage->description_id)
                <p class="hero-description">{!! nl2br(e($memorialPage->description_id)) !!}</p>
            @endif
    ...
    <div class="section" id="family">
        ...
        @if ($memorialPage->wife_name)
            <div class="section-block reveal" data-delay="0">
                <p class="s-label">Istri:</p>
                <p class="s-value">{{ $memorialPage->wife_name }}</p>
            </div>
        @endif

        @if ($memorialPage->children && count($memorialPage->children) > 0)
            <div class="section-block reveal" data-delay="80">
                <p class="s-label">Anak-anak:</p>
                @foreach ($memorialPage->children as $child)
                    <p class="s-value sm">{{ $child }}</p>
                @endforeach
            </div>
        @endif
    ...
    <div class="section" id="funeral">
        ...
        @if ($memorialPage->schedule_closing_coffin && $memorialPage->schedule_closing_coffin !== '—')
            <div class="section-block reveal" data-delay="80">
        ...
        @if ($memorialPage->schedule_comfort_service && $memorialPage->schedule_comfort_service !== '—')
            <div class="section-block reveal" data-delay="160">
                <p class="s-label">Ibadah Penghiburan</p>
                @foreach (explode("\n", $memorialPage->schedule_comfort_service) as $line)
                    <p class="s-value">{{ trim($line) }}</p>
                @endforeach
            </div>
        @endif

        @if ($memorialPage->schedule_departure_service && $memorialPage->schedule_departure_service !== '—')
            <div class="section-block reveal" data-delay="320">
                <p class="s-label">Ibadah Pemberangkatan</p>
                @foreach (explode("\n", $memorialPage->schedule_departure_service) as $line)
                    <p class="s-value">{{ trim($line) }}</p>
                @endforeach
            </div>
        @endif
